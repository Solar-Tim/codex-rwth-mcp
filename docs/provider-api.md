...
RWTH LLM Hosting is documented here as one example provider used for the
initial setup.
...
WestAI, or any provider institution. It is a student-maintained MCP server that
can connect to OpenAI-compatible providers when the user already has authorized
access and a permitted use case.
